...
INSERT INTO `t_mascota` VALUES (1,1,'perro','roko','2022-03-10','Pastor Aleman','grande','macho','El perro se encuentra saludable'),(5,1,'Perro','maya','2022-12-06','Pitbull','Mediano','Hembra','El perro se encuentra saludable');
...
INSERT INTO `t_persona` VALUES (1,'juan','de la cruz','cruz','5612994015'),(2,'Eduardo','De la cruz','cruz','5535141320');
...
INSERT INTO `t_vacunas` VALUES (1,1,1,'distemper','2022-11-27'),(2,5,1,'Polivalente','2022-12-07');
...
/*!50001 VIEW `v_desparasitacion` AS select `mascota`.`id_mascota` AS `idMascota`,`usuario`.`id_usuario` AS `idUsuario`,`mascota`.`nombre` AS `nombreMascota`,`mascota`.`tipo` AS `tipo`,`desparasitacion`.`fecha` AS `fecha`,(select concat(`persona`.`paterno`,' ',`persona`.`materno`,' ',`persona`.`nombre`) from `t_persona` `persona` where `persona`.`id_persona` = `mascota`.`id_persona`) AS `nombrePersona`,concat(`persona`.`paterno`,' ',`persona`.`materno`,' ',`persona`.`nombre`) AS `nombreUsuario` from (((`t_desparasitacion` `desparasitacion` join `t_usuario` `usuario` on(`usuario`.`id_usuario` = `desparasitacion`.`id_usuario`)) join `t_mascota` `mascota` on(`mascota`.`id_mascota` = `desparasitacion`.`id_mascota`)) join `t_persona` `persona` on(`usuario`.`id_persona` = `persona`.`id_persona`)) */;
...
-- Dump completed on 2022-12-11  7:52:31

...
INSERT INTO `t_desparasitacion` VALUES (1,1,1,'2022-11-24');
...
/*!50001 VIEW `v_desparasitacion` AS select `mascota`.`id_mascota` AS `idMascota`,`usuario`.`id_usuario` AS `idUsuario`,`mascota`.`nombre` AS `nombreMascota`,`mascota`.`tipo` AS `tipo`,`desparasitacion`.`fecha` AS `fecha`,(select concat(`persona`.`paterno`,' ',`persona`.`materno`,' ',`persona`.`nombre`) from `t_persona` `persona` where `persona`.`id_persona` = `mascota`.`id_persona`) AS `nombrePersona` from ((`t_desparasitacion` `desparasitacion` join `t_usuario` `usuario` on(`usuario`.`id_usuario` = `desparasitacion`.`id_usuario`)) join `t_mascota` `mascota` on(`mascota`.`id_mascota` = `desparasitacion`.`id_mascota`)) */;
...
-- Dump completed on 2022-12-01 14:45:44
